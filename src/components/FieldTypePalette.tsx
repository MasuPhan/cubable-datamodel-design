...
export const FieldTypePalette = ({ setIsDraggingField }) => {
  ...
  const fieldTypeCategories = {
    all: fieldTypes,
    basic: fieldTypes.filter(f => ["id", "text", "longText", "email", "number", "boolean", "date", "select"].includes(f.value)),
    advanced: fieldTypes.filter(f => ["image", "file", "json", "formula", "richText", "url", "phone", "currency"].includes(f.value)),
    reference: fieldTypes.filter(f => ["reference", "referenceTwo"].includes(f.value))
  };
